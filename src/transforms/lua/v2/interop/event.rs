use crate::event::{Event, LogEvent, Metric, cloud_event::CloudEvent};
use rlua::prelude::*;
...
impl<'a> ToLua<'a> for Event {
    fn to_lua(self, ctx: LuaContext<'a>) -> LuaResult<LuaValue> {
        ...
        match self {
            ...
            Event::Cloud(event) => table.set("cloud_event", event.to_lua(ctx)?)?,
        }
...
impl<'a> FromLua<'a> for Event {
    fn from_lua(value: LuaValue<'a>, ctx: LuaContext<'a>) -> LuaResult<Self> {
        ...
        match (table.get("log")?, table.get("metric")?, table.get("cloud_event")?) {
            ...
            (LuaValue::Nil, LuaValue::Nil, LuaValue::Table(cloud)) => Ok(Event::Cloud(CloudEvent::from_lua(
                LuaValue::Table(cloud),
                ctx,
            )?)),
